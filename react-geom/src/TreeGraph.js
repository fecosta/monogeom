...
const transitionsSetUp = (link, linkEnter, diagonal, transition, source, root) => {
...
const linksGenerator = (root, gLinkRef, margin, source, diagonal, setTooltip) => {
...
const nodesGenerator = (showBubble, nodesThreshold, setHoveredNode, setInactiveNodes, nodes, source, gNodeRef, margin, update, setTooltip, color, transition, generatorSizeNode) => {
  // Update the nodes…
  ...
  const nodeEnter = node
    ...
    .attr("transform", (d) => `translate(${source.x0},${source.y0 + margin.top})`)
...
const printPlot = (data, setTooltip, inactiveNodes, gLinkRef, showBubble, nodesThreshold, svg, gNodeRef, setHoveredNode, setInactiveNodes, refConteiner) => {
  ...
  function update(event, source, duration = 500) {
    ...
    nodesGenerator(showBubble, nodesThreshold, setHoveredNode, setInactiveNodes, nodes, source, gNodeRef, margin, update, setTooltip, color, transition, generatorSizeNode);
    const { link, linkEnter } = linksGenerator(root, gLinkRef, margin, source, diagonal, setTooltip);

    //new function transitions
    transitionsSetUp(link, linkEnter, diagonal, transition, source, root);
  }
